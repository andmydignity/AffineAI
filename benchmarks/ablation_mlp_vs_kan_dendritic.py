#!/usr/bin/env python3
"""
Deep Architectural Ablation: Beyond MLPs for Rapid & Data-Efficient Learning
============================================================================
Tests alternative non-linear channel transformations against standard MLPs:
  - Arm 0: Standard SwiGLU MLP (Baseline: W2(SiLU(W1 x) * W3 x))
  - Arm 1: Rational KAN (1D Padé Rational Polynomial functions on channels)
  - Arm 2: Dendritic Sigma-Pi (Direct multiplicative product units for conjunctions)
  - Arm 3: Dendritic Rational KAN (Multiplicative conjunctions of 1D rational functions)
...
# ---------------------------------------------------------------------------
# Channel Transformation Architectures Beyond MLPs
# ---------------------------------------------------------------------------

class StandardSwiGLUMixer(nn.Module):
    """Arm 0: Standard MLP / SwiGLU baseline."""
...
class RationalKANMixer(nn.Module):
    """
    Arm 1: Rational Kolmogorov-Arnold Network (KAN) Channel Mixer.
    Replaces static scalar non-linearities with learnable 1D Padé rational functions.
    f(x) = (a1*x + a2*x^2 + a3*x^3) / (1 + |b1|*|x| + |b2|*x^2).
    Has compact local support and avoids global hyperplane interference.
    """
    def __init__(self, dim: int, expand: int = 2):
        super().__init__()
        self.dim = dim
        self.hidden = dim * expand
        self.w_in = BitLinear(dim, self.hidden, bias=False)
        # Learnable rational polynomial coefficients per channel
        self.a1 = nn.Parameter(torch.ones(self.hidden) * 0.7)
        self.a2 = nn.Parameter(torch.randn(self.hidden) * 0.02)
        self.a3 = nn.Parameter(torch.randn(self.hidden) * 0.01)
        self.b1 = nn.Parameter(torch.zeros(self.hidden))
        self.b2 = nn.Parameter(torch.zeros(self.hidden))
        self.norm = RMSNorm(self.hidden)
        self.w_out = BitLinear(self.hidden, dim, bias=False)

    def forward(self, x):
        h = self.w_in(x)
        # 1D Rational function evaluated along each feature coordinate
        h_sq = h * h
        num = self.a1 * h + self.a2 * h_sq + self.a3 * (h_sq * h)
        den = 1.0 + self.b1.abs() * h.abs() + self.b2.abs() * h_sq
        act = self.norm(num / (den + 1e-4))
        return self.w_out(act)


class DendriticSigmaPiMixer(nn.Module):
    """
    Arm 2: Dendritic Multiplicative Gating (Sigma-Pi Product Units).
    Biological pyramidal neurons perform multiplicative conjunctions on dendrites.
    Computes direct feature products (h1 * h2) to learn syntactic conjunctions in 1 step.
    """
    def __init__(self, dim: int, expand: int = 2):
        super().__init__()
        self.hidden = dim * expand
        self.w_in = BitLinear(dim, self.hidden * 2, bias=False)
        self.norm = RMSNorm(self.hidden)
        self.w_out = BitLinear(self.hidden, dim, bias=False)
    ...
    def forward(self, x):
        ...
        # Multiplicative conjunction of dendritic clusters
        dendritic = self.norm(h1 * h2)
        return self.w_out(dendritic)


class DendriticRationalKANMixer(nn.Module):
    """
    Arm 3: The Synthesized Dendritic Rational KAN.
    Combines 1D rational polynomial basis functions with multiplicative dendritic gating:
    y = W_out( Rational_1(h1) * Rational_2(h2) )
    """
    def __init__(self, dim: int, expand: int = 2):
        super().__init__()
        self.hidden = dim * expand
        ...
        self.a1 = nn.Parameter(torch.ones(self.hidden * 2) * 0.7)
        self.a2 = nn.Parameter(torch.randn(self.hidden * 2) * 0.02)
        self.b1 = nn.Parameter(torch.zeros(self.hidden * 2))
    ...
    def forward(self, x):
        h = self.w_in(x)
        h_sq = h * h
        num = self.a1 * h + self.a2 * h_sq
        den = 1.0 + self.b1.abs() * h.abs()
        act = num / (den + 1e-4)
        
        h1, h2 = act.chunk(2, dim=-1)
        dendritic = self.norm(h1 * h2)
        return self.w_out(dendritic)


# ---------------------------------------------------------------------------
# Trunk, Decoder & Model Scaffold
# ---------------------------------------------------------------------------
...
class HybridEncoderBlock(nn.Module):
    def __init__(self, dim: int, n_heads: int = 4, layer_idx: int = 0, channel_mixer_type: str = "swiglu"):
        super().__init__()
        ...
        if channel_mixer_type == "swiglu":
            self.channel_mixer = StandardSwiGLUMixer(dim, expand=2)
        elif channel_mixer_type == "rational_kan":
            self.channel_mixer = RationalKANMixer(dim, expand=2)
        elif channel_mixer_type == "dendritic_sigmapi":
            self.channel_mixer = DendriticSigmaPiMixer(dim, expand=2)
        elif channel_mixer_type == "dendritic_kan":
            self.channel_mixer = DendriticRationalKANMixer(dim, expand=2)
        else:
            raise ValueError(f"Unknown channel_mixer_type: {channel_mixer_type}")
...
class ComparativeArchitectureModel(nn.Module):
    def __init__(
        self,
        vocab_size: int = 256,
        dim: int = 128,
        ...
        self.blocks = nn.ModuleList([
            HybridEncoderBlock(dim=dim, n_heads=n_heads, layer_idx=i, channel_mixer_type=channel_mixer_type)
    ...
    def forward(self, byte_ids, targets=None):
        ...
        # Word-boundary aligned patch pooling (Arm 4 standard)
...
def run_experiment_arm(
    ...
    model = ComparativeArchitectureModel(
        dim=128,
        d_byte=64,
        n_encoder_layers=4,
        n_heads=4,
    ...
    print(f"    Params: {param_count:,d} | Channel Mixer: {channel_mixer_type}")
    ...
    for step in range(1, steps + 1):
        ...
        if step == 1:
            cm = model.blocks[0].channel_mixer
            print(f"    [Step 1 Gradient Parity Check]: Channel mixer params active in autograd.")
...
def main():
    ...
    print("=" * 85)
    print(f"   BEYOND MLPs: DEEP ARCHITECTURAL DATA-EFFICIENCY ABLATION (Device: {device})")
    ...
    arms = [
        ("Arm 0: Standard SwiGLU MLP (Baseline)", "swiglu"),
        ("Arm 1: Rational KAN (1D Padé Polynomial Functions)", "rational_kan"),
        ("Arm 2: Dendritic Sigma-Pi (Multiplicative Gating Units)", "dendritic_sigmapi"),
        ("Arm 3: Dendritic Rational KAN (Multiplicative Rational Units)", "dendritic_kan"),
    ]

    results = []
    for name, mixer_type in arms:
        res = run_experiment_arm(
            arm_name=name,
            channel_mixer_type=mixer_type,
    ...
    print("\n" + "=" * 95)
    print("              FINAL NON-MLP ARCHITECTURAL ABLATION RESULTS (100MB)")
    print("=" * 95)
