#!/usr/bin/env python3
"""
Long-Horizon High-Exposure Benchmark (10 MB Data, SeqLen 1024, 1200 Steps)
==========================================================================
Breaks the 1.6MB / 46-word bottleneck by scaling to:
  1. Data Exposure: 1,200 steps x 8 x 1024 = ~10.0 MB (6x more data)
  2. Context Horizon: seq_len = 1024 bytes (~185 words, full story context)
  3. Patch-Level Macro-Modeling vs Non-Parametric Memory vs Tensor Networks

Arms Evaluated:
  - Arm 0: Standard SwiGLU MLP (Neuron Baseline, 486k)
  - Arm 1: Quantum Tensor Network (MPS, 250k)
  - Arm 2: Episodic Associative Memory (Modern Hopfield, 258k)
  - Arm 3: Dual-Horizon Hierarchical Rollout (Macro Speculation, 291k)
  - Arm 4: Rational KAN (Padé Polynomials, 361k)
...
class RationalKANMixer(nn.Module):
    """Padé [3/2] Rational Function Non-Linear Channel Mixer."""
    def __init__(self, dim: int = 128, expand: int = 2):
        super().__init__()
        self.dim = dim
        self.hidden = dim * expand # 256
        self.w_in = BitLinear(dim, self.hidden, bias=False)
        self.a1 = nn.Parameter(torch.ones(self.hidden) * 1.0)
        self.a2 = nn.Parameter(torch.randn(self.hidden) * 0.1)
        self.a3 = nn.Parameter(torch.randn(self.hidden) * 0.05)
    ...
    def forward(self, x):
        h = self.w_in(x)
        num = self.a1 * h + self.a2 * (h ** 2) + self.a3 * (h ** 3)
        den = 1.0 + torch.abs(self.b1) * torch.abs(h) + torch.abs(self.b2) * (h ** 2)
        act = num / den.clamp(min=1e-4)
        return self.w_out(self.norm(act.to(x.dtype)))


# ---------------------------------------------------------------------------
# Scaffold Blocks & Decoders
# ---------------------------------------------------------------------------
...
class HybridEncoderBlock(nn.Module):
    def __init__(self, dim: int, n_heads: int = 4, layer_idx: int = 0, mixer_type: str = "mps"):
        super().__init__()
        ...
        if mixer_type == "swiglu":
            self.channel_mixer = SwiGLUMixer(dim, hidden=256)
        elif mixer_type == "mps":
...
class EpisodicHopfieldMemory(nn.Module):
    """Modern Continuous Hopfield Non-Parametric Associative Memory."""
    ...
    def forward(self, h2_norm: torch.Tensor, base_logits: torch.Tensor, targets: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        tgt_exp = targets.unsqueeze(1).expand(-1, T, -1)
...
class TemporalByteDecoder(nn.Module):
    def __init__(self, vocab_size: int = 256, d_byte: int = 64, d_model: int = 128, conv_k: int = 8, use_speculation: bool = False):
        ...
        in_dim = 2 * d_model if use_speculation else d_model
        self.patch_to_byte = BitLinear(in_dim, d_byte, bias=False)
...
class LongHorizonEvaluationModel(nn.Module):
    def __init__(
        self,
        vocab_size: int = 256,
        dim: int = 128,
        ...
        self.blocks = nn.ModuleList([
            HybridEncoderBlock(dim=dim, n_heads=n_heads, layer_idx=i, mixer_type=mixer_type)
    ...
    def forward(self, byte_ids, targets=None):
        ...
        spec_patches = None
        if self.use_speculation and self.macro_speculator is not None:
        ...
        if self.use_hopfield and self.hopfield_memory is not None:
...
def run_experiment_arm(
    ...
    model = LongHorizonEvaluationModel(
        dim=128,
        d_byte=64,
        n_encoder_layers=4,
        n_heads=4,
    ...
    print(f"    Params: {param_count:,d} | Mixer: {mixer_type} | Hopfield: {use_hopfield} | Speculation: {use_speculation}")
...
def main():
    ...
    print("=" * 95)
    print(f"   LONG-HORIZON HIGH-EXPOSURE BENCHMARK (10.0 MB, SeqLen 1024, 1200 Steps, Device: {device})")
    print("   Dataset: SimpleStories (100.0 MB Active Slice)")
    print("=" * 95)
    ...
    arms = [
        # Arm 0: Standard SwiGLU MLP (Neuron Baseline, 486k)
        ("Arm 0: Standard SwiGLU MLP (Baseline)", "swiglu", False, False),

        # Arm 1: Quantum Tensor Network (MPS, 250k)
        ("Arm 1: Quantum Tensor Network (MPS)", "mps", False, False),

        # Arm 2: Episodic Associative Memory (Modern Hopfield, 258k)
        ("Arm 2: Episodic Associative Memory (Hopfield)", "mps", True, False),

        # Arm 3: Dual-Horizon Hierarchical Rollout (Macro Speculation, 291k)
        ("Arm 3: Dual-Horizon Rollout (Macro Speculation)", "mps", False, True),

        # Arm 4: Rational KAN (Padé Polynomials, 361k)
        ("Arm 4: Rational KAN (Padé Polynomials)", "kan", False, False),
    ]

    results = []
    for name, m_type, hopfield, spec in arms:
    ...
    print("\n" + "=" * 105)
    print("        FINAL RESULTS: LONG-HORIZON HIGH-EXPOSURE BENCHMARK (10.0 MB, SeqLen 1024, 1200 Steps)")
    print("=" * 105)
