...
def main():
    ...
    print("=" * 95)
    print(f"          TOROS-HYBRID END-TO-END TINYSTORIES TRAINING (Device: {device})")
    print("=" * 95)

    data_path = "data/tinystories_full_eos.bin"
    file_size = os.path.getsize(data_path)
    print(f"Loading Full Dataset from: {data_path} ({file_size:,d} bytes, {file_size / (1024**3):.2f} GB)")
    ...
    print(f"Train Set Size      : {len(train_data):,d} bytes (3.67 GB)")
    print(f"Validation Set Size : {len(val_data):,d} bytes (193.5 MB)")
    print("-" * 95)
    ...
    best_val_ppl = 999.0
    save_path = "models/toros_hybrid_tinystories_best_clean.pt"
    t0 = time.time()
    ...
    prompts = [
        ...
        "Tim and his dog Spot went into the deep forest to find a hidden treasure."
    ]

    print("\nSAMPLE STORY GENERATIONS (Stopping at <|endoftext|>):")
    print("=" * 95)
