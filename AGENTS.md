...
- Throughput sweet spot is B10-12 on 8 physical cores (~250k tok/s at dim96/d32 with tcmalloc via `source env_cpu.sh`); B16+ adds nothing without more cores. Mild oversubscription helps the memory-bound phases.
...
- `affine_ai/core/` — layers and engines: `ast_dag.py` (ASDAG layer), `backpressure_tree.py` (`FusedSparseBackpressureTreeV3` — deprecated in favor of `AdaptiveSparseTreeDAGLayer` in `ast_dag.py`), `cpp_ops.py` (C++ bindings, all have PyTorch fallbacks), `format.py` (custom `.toros` binary checkpoint format; loading compressed files requires the `zstandard` package, **not** in pyproject), `lpc.py` (Local Predictive Coding forward-only training), `rls_head.py` (RLS closed-form posterior head), `type_codebook.py` (type codebook + BMR + info-gain), `growth.py` (stick-breaking growth controller).
- `affine_ai/models/` — `language_model.py` (ASDAG LM, default `channel_mixer_type="ternary_swiglu"`), `blt.py`, `hybrid.py` (stripped: patch encoder + BLT decoder, no JEPA), `jepa.py` (`TorosJEPA` — deprecated), `mtp.py`.
- **JEPA is DEPRECATED**: `TorosJEPA` / `TorosPredictor` are deprecated. JEPA/System-2 is stripped from hybrid: no `predictor`/`local_heads`/`mask_token`/`forward_lpc_step`; `forward()` is gen-only (+ optional unlikelihood). JEPA is EMA-free (LeJEPA-style) where it exists (`jepa.py` standalone); `target_encoder`/`update_target_encoder` are stale. The predictor was training-only scaffolding and old checkpoints with it load via `strict=False`.
