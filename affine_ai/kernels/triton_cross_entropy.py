...
def _fused_linear_cross_entropy_fwd_kernel(
    ...
    for v_start in range(0, V, BLOCK_V):
        ...
        for d_start in range(0, D, BLOCK_D):
            ...
            acc += tl.dot(h_tile, tl.permute(w_tile, (1, 0)), allow_tf32=False)
        ...
        diff = tl.minimum(acc - m_new[:, None], 0.0)
...
def _fused_linear_cross_entropy_bwd_dh_kernel(
    ...
    # Inverted loop structure: outer loop over V computes logits ONCE per (M, V) block
    for v_start in range(0, V, BLOCK_V):
        offs_v = v_start + tl.arange(0, BLOCK_V)
        mask_v = offs_v < V
        ...
        for d_k in range(0, D, BLOCK_D):
            ...
            logits += tl.dot(h_k, tl.permute(w_k, (1, 0)), allow_tf32=False)

        diff = tl.minimum(logits - lse[:, None], 0.0)
        p = tl.where(valid_mask[:, None] & mask_v[None, :], tl.exp(diff), 0.0)
        is_target = (target[:, None] == offs_v[None, :]) & valid_mask[:, None] & mask_v[None, :]
        dlogits = tl.where(valid_mask[:, None] & mask_v[None, :], p - tl.where(is_target, 1.0, 0.0), 0.0)
        # Native operand dtype enables hardware Tensor Cores for subsequent projection
        scaled_acc = (dlogits * grad_scale).to(W_ptr.dtype.element_ty)

        for d_start in range(0, D, BLOCK_D):
            offs_d = d_start + tl.arange(0, BLOCK_D)
            mask_d = offs_d < D
            w_d = tl.load(W_ptr + offs_v[:, None] * stride_wv + offs_d[None, :] * stride_wd, mask=mask_v[:, None] & mask_d[None, :], other=0.0)
            dh_contrib = tl.dot(scaled_acc, w_d, allow_tf32=False)
...
def _fused_linear_cross_entropy_bwd_dw_kernel(
    ...
    # Inverted loop structure: outer loop over N computes logits ONCE per (V, N) block
    for n_start in range(0, N, BLOCK_N):
        ...
        for d_k in range(0, D, BLOCK_D):
            ...
            logits += tl.dot(w_k, tl.permute(h_k, (1, 0)), allow_tf32=False)

        diff = tl.minimum(logits - lse[None, :], 0.0)
        p = tl.where(mask_v[:, None] & valid_mask[None, :], tl.exp(diff), 0.0)
        ...
        # Native operand dtype enables hardware Tensor Cores for subsequent projection
        scaled_acc_dw = (dlogits * grad_scale).to(H_ptr.dtype.element_ty)

        for d_start in range(0, D, BLOCK_D):
            offs_d = d_start + tl.arange(0, BLOCK_D)
            mask_d = offs_d < D
            h_d = tl.load(H_ptr + offs_n[:, None] * stride_hb + offs_d[None, :] * stride_hd, mask=mask_n[:, None] & mask_d[None, :], other=0.0)
            dw_contrib = tl.dot(scaled_acc_dw, h_d, allow_tf32=False)
