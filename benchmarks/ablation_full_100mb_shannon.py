#!/usr/bin/env python3
"""
Full 100MB SimpleStories Training: Reaching the Shannon Limit
============================================================
Trains on the ENTIRE 100.0 MB dataset (95 MB train, 5 MB val) with full dataset coverage:
  - Batch size = 128, seq_len = 1024 (131,072 bytes per step)
  - Full epoch over 95 MB = ~740 steps; total 1,000 steps (~131 MB seen, 1.35 epochs)
  - Throughput: ~550,000 tokens/sec on NVIDIA RTX 3050 GPU with HybridMuonAdamW

Evaluates:
  - Arm 0: Baseline Reflex (Standard Feed-Forward, 250k params)
  - Arm 1: Persistent Entity-Slot Memory (K=8 Invariant Causal Slots, 292k params)
  - Arm 2: Continuous Latent Reasoning (Coconut K=2 Deliberation, 315k params)
...
# ---------------------------------------------------------------------------
# Specialized Inner State Modules
# ---------------------------------------------------------------------------
...
class PersistentEntitySlotMemory(nn.Module):
    ...
    def forward(self, s: torch.Tensor) -> torch.Tensor:
        B, M, D = s.shape
        slots = self.init_slots.expand(B, -1, -1).clone().to(s.dtype)
        slot_hist = []

        for m in range(M):
            sm = s[:, m : m + 1, :] # [B, 1, D]
            scores = torch.bmm(slots, sm.transpose(1, 2)) / math.sqrt(D)
            attn = F.softmax(scores, dim=1)
            delta = self.slot_update(sm)
            slots = self.norm_slot(0.85 * slots + 0.15 * attn * delta)
            slot_summary = torch.bmm(attn.transpose(1, 2), slots)
            slot_hist.append(slot_summary)

        entity_trajectory = torch.cat(slot_hist, dim=1)
...
class Full100MBModel(nn.Module):
    def __init__(
        self,
        vocab_size: int = 256,
        dim: int = 128,
        ...
        if paradigm == "latent_deliberation":
    ...
    def forward(self, byte_ids, targets=None):
        ...
        if self.paradigm == "latent_deliberation":
...
# ---------------------------------------------------------------------------
# Training Harness with Systematic Full-Dataset Coverage
# ---------------------------------------------------------------------------

def run_full_100mb_arm(
    arm_name: str,
    paradigm: str,
    ...
    total_bytes_to_see = total_steps * bytes_per_step
    print(f"    Params: {param_count:,d} | Batch: {batch_size} | SeqLen: {seq_len} ({bytes_per_step:,d} bytes/step)")
    print(f"    Target Exposure: {total_bytes_to_see / (1024*1024):.1f} MB ({(total_bytes_to_see / len(train_data)):.2f} Epochs of 95MB Train Data)")
    ...
    def update_lr(opt, s):
        if s < warmup_steps:
            ...
            ratio = max(0.08, 0.5 * (1.0 + math.cos(math.pi * progress)))
    ...
    # Build systematic validation batches across the 5MB val data
    val_batches = []
    np.random.seed(1337)
    high_val = len(val_data) - seq_len - 1
    for _ in range(8):
        ix = np.random.randint(0, high_val, size=16) # batch 16 for val
    ...
    print(f"    Step     0 ( 0.0 MB) | Val Loss: {init_loss:.4f} | Val PPL: {init_ppl:.2f} | Val BPC: {init_bpc:.3f}")

    # Shuffled chunk indexer covering all 95MB of training data systematically
    num_chunks = (len(train_data) - seq_len - 1) // seq_len
    chunk_offsets = np.arange(num_chunks) * seq_len
    ...
    for step in range(1, total_steps + 1):
        ...
        # Gather batch of chunks from the systematic permutation
        if perm_ptr + batch_size > len(current_permutation):
            # Epoch completed: reshuffle for next epoch pass
            remaining = current_permutation[perm_ptr:]
            rng_data = np.random.RandomState(42 + step)
            current_permutation = rng_data.permutation(chunk_offsets)
        ...
        if step % 100 == 0 or step == total_steps:
            ...
            print(f"    Step {step:4d}/{total_steps} ({data_seen_mb:5.1f}MB, {epoch_frac:4.2f}ep) | Train Loss: {loss_val:.4f} | Val Loss: {val_loss:.4f} | Val PPL: {val_ppl:5.2f} | BPC: {val_bpc:.3f} | {tok_per_sec:7,.0f} tok/s")

    # In-distribution sampling probe with 128-byte prompt from validation slice
    ...
    with torch.no_grad():
        ...
        for _ in range(128):
            ...
            next_token = sample_next_token(logits[:, -1, :], temperature=0.75, top_k=40, top_p=0.9, generator=rng_gen)
...
def main():
    ...
    print("=" * 105)
    print(f"   FULL 100MB DATASET BENCHMARK: TESTING THE THEORETICAL SHANNON LIMIT (Device: {device})")
    print("   Dataset: SimpleStories (100.0 MB Active Slice, 95MB Train / 5MB Val)")
    print("   Protocol: Batch Size 128, SeqLen 1024, 1,000 Steps (~131 MB Exposure, 1.35 Epochs)")
    print("=" * 105)
    ...
    arms = [
        # Arm 0: Baseline Feedforward Reflex (250k)
        ("Arm 0: Baseline Reflex (No Inner State, 250k)", "baseline"),

        # Arm 1: Persistent Entity-Slot Memory (292k)
        ("Arm 1: Persistent Entity-Slot Memory (Slots, 292k)", "slot_memory"),

        # Arm 2: Continuous Latent Reasoning (Coconut, 315k)
        ("Arm 2: Continuous Latent Reasoning (Coconut, 315k)", "latent_deliberation"),
    ]

    results = []
    for name, p_type in arms:
        res = run_full_100mb_arm(
            arm_name=name,
            paradigm=p_type,
    ...
    print("\n" + "=" * 110)
    print("          FINAL 100MB SHANNON LIMIT RESULTS (FULL DATASET EXPOSURE)")
    print("=" * 110)
    print(f"{'Arm':<54} | {'Params':<8} | {'Val PPL':<8} | {'Val BPC':<8} | {'ms/step':<8} | {'Dist-4g':<8}")
    print("-" * 110)
    for r in results:
        print(f"{r['arm']:<54} | {r['params']:<8,d} | {r['final_val_ppl']:<8.2f} | {r['val_bpc']:<8.3f} | {r['avg_ms_step']:<8.1f} | {r['distinct_4gram']:<8.3f}")
    print("=" * 110)
