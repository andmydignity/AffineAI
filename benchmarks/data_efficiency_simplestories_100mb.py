#!/usr/bin/env python3
"""
TorosHybrid Data-Efficiency Ablation on SimpleStories (100MB Cap)
================================================================
Evaluates inductive mechanisms on the SimpleStories distribution:
  - Arm 0: Baseline (Current TorosHybrid Defaults: Trunk Conv Prefix k=4 + Dense Readout)
  - Arm 1: Decoder Temporal Continuity (k=8 Causal Conv in ByteLocalDecoder)
  - Arm 2: Pyramidal Dilated Convs (Dilations 1, 2, 4, 8 across trunk blocks)
  - Arm 3: Word-Boundary Aligned Patching (Entropy/delimiter-weighted patch pooling)
  - Arm 4: Combined Best (Decoder Conv k=8 + Word-Boundary Aligned Patching)

Hardware: NVIDIA RTX 3050 GPU with Triton Hardware Kernels.
"""
...
class TestTorosHybridModel(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            HybridEncoderBlock(dim=dim, n_heads=n_heads, dilation=dilations[i], layer_idx=i, use_conv_prefix=use_conv_prefix)
...
def run_experiment_arm(
    ...
    print(f"      - Pyramidal Dilations     : {model_kwargs.get('use_pyramidal_dilations', False)}")
...
def main():
    ...
    print("=" * 85)
    print(f"   TOROS-HYBRID DATA-EFFICIENCY ABLATION ON SIMPLESTORIES (Device: {device})")
    ...
    arms = [
        ...
        (
            "Arm 4: Combined Best (Decoder Conv + Word-Aligned)",
            {"dim": DIM, "d_byte": D_BYTE, "n_encoder_layers": N_LAYERS, "n_heads": N_HEADS,
             "use_conv_prefix": True, "use_dense_readout": True,
             "use_decoder_temporal_conv": True, "use_pyramidal_dilations": False, "use_word_aligned_patching": True}
        ),
    ]
    ...
    print("\n" + "=" * 95)
    print("              FINAL ABLATION RESULTS SUMMARY (SIMPLESTORIES 100MB)")
    print("=" * 95)
    print(f"{'Arm':<50} | {'Params':<8} | {'Val PPL':<8} | {'Val BPC':<8} | {'ms/step':<8} | {'Dist-4g':<8}")
    print("-" * 95)
    for r in results:
        print(f"{r['arm']:<50} | {r['params']:<8,d} | {r['final_val_ppl']:<8.2f} | {r['val_bpc']:<8.3f} | {r['avg_ms_step']:<8.1f} | {r['distinct_4gram']:<8.3f}")
    print("=" * 95)
